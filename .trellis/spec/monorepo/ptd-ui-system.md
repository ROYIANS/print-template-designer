...
- **Frame/node**：Canvas viewport 内允许一层细线工作区内框；四角与 App Bar/左工作区主结构线交点
  可使用 7px、45° 旋转的空心方形节点。节点是结构交点，不是 Logo，也不使用校样朱红。
...
- **Ruler/ticks**：标尺与刻度必须对应实际位置或尺寸；纯装饰刻线只能出现在不干扰读数的 chrome 边界。
...
- Paper shadow 由 1px 硬边框、约 4px 右下实体偏移与一层柔和长阴影组成，表达装配在工作台上的
  纸张；阴影不能复制到面板或组件卡片。
- Pasteboard 可以同时包含低对比斜线材质、24px 工程网格、工作区内框、四角节点和纸张边缘
  无数字刻线；所有装饰必须停留在应用 Chrome/Pasteboard，绝不能进入 Paper 内容或导出结果。
- 标尺是工具，不是装饰：刻度低对比，主刻度和当前指针读数清晰，数字使用 tabular nums。
- 页面边距使用弱朱红虚线；选中框、控制点和参考线使用主题钴蓝。钴蓝只服务于交互，
  不作为面板或画布的大面积底色。

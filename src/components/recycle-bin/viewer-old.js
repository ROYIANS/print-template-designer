...
export const renderMain = {
  ...
  async renderRoyComplexTable({
    ...
    const { COMMON_SCALE } = CONFIG
    let StyledComplexTableConstructor = Vue.extend(StyledComplexTable)
    let newPage = pageSet.curPage
    if (newPage === null) {
      newPage = await renderMain.renderPage(pagerConfig, pageSet)
    }
    ...
    const getNewElement = (isNewPage) => {
      const instanceTable = new StyledComplexTableConstructor({
        propsData: style
      })
      instanceTable.$mount()
      const newTable = instanceTable.$el
      ...
      newTable.style.position = 'absolute'
      newTable.style.left = `${style.left}px`
      newTable.style.top = `${isNewPage ? style.top : realPageMarginTop}px`
      newTable.style.transform = `rotate(${style.rotate}deg)`
      instanceTable.$mount()
    }
